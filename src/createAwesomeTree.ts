...
import { toArray, unwrapCheckedKeys, unwrapIndeterminateKeys } from './check-utils';
...
import { isDisabled, isGroup, isIgnored, isLeaf, isShallowLoaded } from './utils';
...
export function createAwesomeTree<R = RawNode, G = R, I = R>(
  ...
  const otherMethods = {
    contains<Row, Group, Ignore>(this: TreeNode<Row, Group, Ignore>, node: TreeNode<Row, Group, Ignore>): boolean {
      return contains(this, node);
    },
  };
  const nodeProto = Object.assign(getNodeStatusMethods, nodeTraverseMethods, otherMethods);
  ...
  function getPrev(key: Key | null | undefined, _options?: GetPrevNextOptions): TreeNode<R> | null {
    const node = _getNode(key);
    if (!node) {
      return null;
    }
  ...
  function getNext(key: Key | null | undefined, _options?: GetPrevNextOptions): TreeNode<R> | null {
    const node = _getNode(key);
    if (!node) {
      return null;
    }
  ...
  const asTree: AwesomeTree<R, G, I> = {
    ...
    getCheckedKeys(checkedKeys: Key[] | InputMergedKeys | null | undefined, _options: CheckOptions = {}) {
