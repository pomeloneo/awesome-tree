import { mergeKeys, rmKeys, traverseSignal, traverseWithSignalCb } from './check-utils';
import type { AwesomeTree, Key, MergedKeys, TreeNode } from './interface';
import { GetSpreadedCheckedKeySetAfterCheckOptions } from './interface';

/*
 * 默认暂无 disabled 节点，不处理disabled相关
 * */

function getSpreadedCheckedKeySet<R, G, I>(checkedKeys: Key[], asTree: AwesomeTree<R, G, I>): Set<Key> {
...
function getAvailableAscendantNodeSet<R, G, I>(uncheckedKeys: Key[], asTree: AwesomeTree<R, G, I>): Set<Key> {
...
function getSpreadedCheckedKeySetAfterUncheck<R, G, I>(
  ...
  const spreadedCheckedKeySet = getSpreadedCheckedKeySet(currentCheckedKeys, asTree);
  const spreadedKeySetToUncheck = getSpreadedCheckedKeySet(uncheckedKeys, asTree);
  const ascendantKeySet: Set<Key> = getAvailableAscendantNodeSet(uncheckedKeys, asTree);
...
export function getCheckedKeys<R, G, I>(
  ...
  const { checkedKeys, keysToCheck, keysToUncheck, indeterminateKeys, cascade, checkStrategy } = options;
  ...
  if (keysToUncheck !== undefined) {
    spreadedCheckedKeySet = getSpreadedCheckedKeySetAfterUncheck(keysToUncheck, checkedKeys, asTree);
  } else if (keysToCheck !== undefined) {
    spreadedCheckedKeySet = getSpreadedCheckedKeySetAfterCheck(keysToCheck, checkedKeys, asTree);
  } else {
      ...
      for (const levelTreeNode of levelTreeNodes) {
        ...
        if (isChildCheckStrategy && shallowLoaded) {
          levelTreeNode.children!.forEach((childNode) => {
            if (!childNode.isLeaf && childNode.shallowLoaded && syntheticCheckedKeySet.has(childNode.key)) {
        ...
        if (isZeroLevel && isChildCheckStrategy && syntheticCheckedKeySet.has(levelTreeNodeKey)) {
