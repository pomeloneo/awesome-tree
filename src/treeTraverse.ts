import { GetPrevNextOptions, TreeNode } from './interface'
...
export function getFirstUsefulNode<R, G, I>(
...
function walk(
  ...
  function t(node: TreeNode | null): void {
    ...
    if (node === fromNode) {
      ...
      if (
        !node.disabled ||
        (includeDisabled && !node.isGroup && !node.ignored)
      ) {
        endNode = node
        return
      }
...
export const traverseMethods: Pick<
  TreeNode,
  'getChild' | 'getParent' | 'getNext' | 'getPrev'
> = {
