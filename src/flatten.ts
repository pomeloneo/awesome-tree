import type { Key, TreeNode } from './interface';

export function flatten<R, G, I>(treeNodes: Array<TreeNode<R, G, I>>, expandedKeys?: Key[]): Array<TreeNode<R, G, I>> {
  ...
  function t(nodes: Array<TreeNode<R, G, I>>): void {
    nodes.forEach((treeNode) => {
      ...
      if (treeNode.isGroup) {
        t(treeNode.children);
      } else if (expandedKeySet === undefined || expandedKeySet.has(treeNode.key)) {
        t(treeNode.children);
      }
    });
  }
